...
{
    return i2c_master_write_read_device(I2C_MASTER_NUM, 0xD5,  &reg_addr, 1, data, len, I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS);
}
...
esp_err_t lsm6dso_register_write_byte(uint8_t reg_addr, uint8_t data)
{
    ...
    return i2c_master_write_to_device(I2C_MASTER_NUM, 0xD4, write_buf, sizeof(write_buf), I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS);
}
